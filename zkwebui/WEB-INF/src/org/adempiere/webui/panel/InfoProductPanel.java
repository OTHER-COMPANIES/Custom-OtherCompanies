...
public class InfoProductPanel extends InfoPanel implements EventListener
{
	...
	private static final String s_productFrom =
		...
		//Carlos Parada
		//WareHouse Filter
		+ " LEFT OUTER JOIN (Select M_Product_ID,M_WareHouse_ID,Sum(QtyAvailable) QtyAvailable,Sum(QtyOnHand) QtyOnHand,Sum(QtyReserved) QtyReserved From M_Product_Stock_V GROUP BY M_Product_ID,M_WareHouse_ID HAVING Sum(QtyAvailable) <>0 OR Sum(QtyOnHand) <>0 OR Sum(QtyReserved)<>0 ) ms ON  ms.M_Product_ID=p.M_Product_ID ";
	...
	public String getSQLWhere()
	{
		...
		if (getM_WareHouse_ID()>0)
			where.append(" AND ms.M_WareHouse_ID=?");
	...
	protected void setParameters(PreparedStatement pstmt, boolean forCount) throws SQLException
	{
		...
		if (M_Warehouse_ID > 0) {
			pstmt.setInt(index++, M_Warehouse_ID);
			log.fine("M_Product_Category_ID=" + M_Warehouse_ID);
		}
    ...
    /**
	 * Get Ware House Selected
	 * @author <a href="mailto:carlosaparadam@gmail.com">Carlos Parada</a> 11/11/2013, 14:42:50
	 * @return
	 * @return int
	 */
	public int getM_WareHouse_ID() {
		int M_WareHouse_ID = 0;
		ListItem wh = (ListItem)pickWarehouse.getSelectedItem();
		if (wh != null)
			M_WareHouse_ID = Integer.parseInt(wh.getValue().toString());
